...
class Medoo
{
    ...
    protected function dataImplode($data, &$map, $conjunctor)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (preg_match("/^(AND|OR)(\s+#.*)?$/i", $key, $relation_match) && $type === 'array') {
                ...
                ) ? '(' . $this->dataImplode($value, $map, ' ' . $relation_match[1]) . ')' : '(' . $this->innerConjunct(
                                $value,
                                $map,
                                ' ' . $relation_match[1],
                                $conjunctor
                ...
                if (is_int($key) && preg_match(
                    ...
                    if (!empty($match[1])) {
                        $wheres[] = $column . (isset($match['operator']) ? ' ' . $match['operator'] . ' ' : ' = ') . $this->fnQuote(
                                        $key,
                                        $value
                                );
    ...
    protected function columnMap($columns, &$stack)
    {
        ...
        foreach ($columns as $key => $value) {
            if (is_int($key)) {
                ...
                $column_key = !empty($key_match['alias']) ? $key_match['alias'] : preg_replace(
                        '/^[\w]*\./i',
                        '',
                        $key_match['column']
                );
            ...
            foreach ($columns as $key) {
                ...
                if (!isset($data[$key])) {
                    ...
                    switch (gettype($value)) {
                        ...
                        case 'array':
                            $map[$map_key] = [
                                    strpos($key, '[JSON]') === strlen($key) - 6 ? json_encode($value) : serialize(
                                            $value
                                    ),
                                    PDO::PARAM_STR
                            ];
    ...
    public function update($table, $data, $where = null)
    {
        ...
        return $this->exec(
                'UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $fields) . $this->whereClause(
                        $where,
                        $map
                ),
                $map
