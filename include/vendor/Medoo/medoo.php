...
class medoo
{
    ...
    {
        return (strpos($column, '#') === 0 && preg_match('/^[A-Z0-9\_]*\([^)]*\)$/', $string)) ? $string : $this->quote(
                $string
        );
    }
    ...
    protected function data_implode($data, $conjunctor, $outer_conjunctor = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (preg_match("/^(AND|OR)(\s+#.*)?$/i", $key, $relation_match) && $type == 'array') {
                ...
                ) ? '(' . $this->data_implode($value, ' ' . $relation_match[1]) . ')' : '(' . $this->inner_conjunct(
                                $value,
                                ' ' . $relation_match[1],
                                $conjunctor
                        ) . ')';
                    ...
                    if ($operator == '<>' || $operator == '><') {
                        if ($type == 'array') {
                            ...
                            if (is_numeric($value[0]) && is_numeric($value[1])) {
                                ...
                                $wheres[] = '(' . $column . ' BETWEEN ' . $this->quote(
                                                $value[0]
                    ...
                    if ($operator == '~' || $operator == '!~') {
                        ...
                        if (!empty($value)) {
                            ...
                            foreach ($value as $item) {
                                ...
                                $like_clauses[] = $column . ($operator === '!~' ? ' NOT' : '') . ' LIKE ' . $this->fn_quote(
                                                $key,
                                                $item
                                        );
                            }
            ...
            if (isset($where['ORDER'])) {
                ...
                if (is_array($ORDER)) {
                    if (isset($ORDER[1]) && is_array($ORDER[1])) {
                        $where_clause .= ' ORDER BY FIELD(' . $this->column_quote(
                                        $ORDER[0]
    ...
    public function insert($table, $datas)
    {
        ...
        foreach ($datas as $data) {
            ...
            foreach ($data as $key => $value) {
                ...
                switch (gettype($value)) {
                    ...
                    case 'array':
                        ...
                        $values[] = isset($column_match[0]) ? $this->quote(json_encode($value)) : $this->quote(
                                serialize($value)
                        );
                        break;
                    case 'boolean':
            ...
            $this->exec(
                    'INSERT INTO "' . $this->prefix . $table . '" (' . implode(', ', $columns) . ') VALUES (' . implode(
                            $values,
                            ', '
                    ) . ')'
            );
    ...
    public function update($table, $data, $where = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (isset($match[3])) {
                if (is_numeric($value)) {
                    $fields[] = $this->column_quote($match[1]) . ' = ' . $this->column_quote(
                                    $match[1]
                            ) . ' ' . $match[3] . ' ' . $value;
                }
            } else {
        ...
        return $this->exec(
                'UPDATE "' . $this->prefix . $table . '" SET ' . implode(', ', $fields) . $this->where_clause($where)
        );
    }
    ...
    public function replace($table, $columns, $search = null, $replace = null, $where = null)
    {
        if (is_array($columns)) {
            ...
            foreach ($columns as $column => $replacements) {
                foreach ($replacements as $replace_search => $replace_replacement) {
                    $replace_query[] = $column . ' = REPLACE(' . $this->column_quote($column) . ', ' . $this->quote(
                                    $replace_search
            ...
            if (is_array($search)) {
                ...
                foreach ($search as $replace_search => $replace_replacement) {
                    $replace_query[] = $columns . ' = REPLACE(' . $this->column_quote($columns) . ', ' . $this->quote(
                                    $replace_search
                            ) . ', ' . $this->quote($replace_replacement) . ')';
                }
                $replace_query = implode(', ', $replace_query);
        ...
        return $this->exec(
                'UPDATE "' . $this->prefix . $table . '" SET ' . $replace_query . $this->where_clause($where)
        );
    }
