...
class DbConfigurationProvider extends BaseConfigurationProvider implements ConfigurationProvider
{
    ...
    public function loadConfiguration($namespace, $context, $language, $environment, $name)
    {
        ...
        if ($conn->getNumRows(
                        $result
                ) == 0 && $this->activateEnvironmentFallback === true && $environment !== 'DEFAULT'
        ) {
            $environment = 'DEFAULT';
            $select      = 'SELECT `section`, `key`, `value` FROM `' . $table . '`
                           WHERE
    ...
    public function deleteConfiguration($namespace, $context, $language, $environment, $name)
    {
        ...
        if ($affectedRows == 0) {
            throw new ConfigurationException('[DbConfigurationProvider::deleteConfiguration()] ' . 'Configuration with name "' . $this->getConfigName(
                            $name
                    ) . '" cannot be deleted! Please check your ' . 'database configuration, the given parameters, or your environment configuration.');
        }
    }
}
