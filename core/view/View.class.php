...
class View
{
    ...
    {
        !defined('VIEW_ENGINE_PATH') && define('VIEW_ENGINE_PATH', CORE_PATH . '/view/SmvcTemplate.class.php');

        if (VIEW_ENGINE_PATH) {
            Importer::importFileByFullPath(VIEW_ENGINE_PATH);
        }
    ...
    {
        !defined('VIEW_ENGINE') && define('VIEW_ENGINE', 'SmvcTemplate');
        $engine = VIEW_ENGINE;
        $this->setEngine($engine);
        if (!defined('VIEW_ENGINE_CONFIG')) { //TODO 这个需要从新实现
            $viewEngineConfig = array( //视图engine相关配置
        ...
        if ($viewEngineConfig) {
            ...
            foreach ($viewEngineConfig as $key => $value) {
