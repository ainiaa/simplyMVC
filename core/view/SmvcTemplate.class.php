...
class SmvcTemplate
{
        ...
        if (empty($tag)) {
            ...
            switch ($tag_sel) {
                ...
                case 'tpl':
                    ...
                    return '<?php echo $this->tpl_base . "/" . ' . "'$t[file]'" . '; ?>';
                    break;
                case 'lib':
                    $t = $this->get_para(substr($tag, 4), 0);
    ...
    public function html_radios($arr)
    {
        ...
        $out = '';
        foreach ($options AS $key => $val) {
            $out .= $key == $checked ? "<label><input type=\"radio\" name=\"$name\" value=\"$key\" checked>&nbsp;{$val}</label>&nbsp;" : "<label><input type=\"radio\" name=\"$name\" value=\"$key\">&nbsp;{$val}</label>&nbsp;";
        }
    ...
    public function html_page_links($arr)
    {
        ...
        if ($page['page_count'] > 1) {
            ...
            for ($i = $start; $i <= $end; $i++) {
                $out .= ($i != $page['curr_page']) ? "<li><a href='$url_format&amp;page=$i'>" . $i . "</a></li>\n" : '<li class="curr-page">' . $i . "</li>\n";
            }
    ...
    public function html_page_selector($arr)
    {
        ...
        for ($i = 1; $i <= $page['page_count']; $i++) {
            $out .= ($i != $page['curr_page']) ? "<option value='$url_format&amp;page=$i'>$i/$page[page_count]</option>\n" : "<option value='$url_format&amp;page=$i' selected=\"true\">$i/$page[page_count]</option>\n";
        }
