...
class BaseDAO extends Object
{
    ...
    public function __construct($name = '', $tablePrefix = '', $connection = '')
    {
        ...
        if (is_null($tablePrefix)) { // 前缀为Null表示没有前缀
            ...
            !defined('DB_PREFIX') && define('DB_PREFIX', 'smvc_');
    ...
    public function __call($method, $args)
    {
        if (in_array(strtolower($method), $this->methods, true)) {
            ...
            // 根据某个字段获取记录
            $field         = parse_name(substr($method, 5)); //todo 这个需要处理
            $where[$field] = $args[0];
            return $this->where($where)->find(); //todo 这个需要处理
        } elseif (strtolower(substr($method, 0, 10)) == 'getfieldby') {
            // 根据某个字段获取记录的某个值
            $name         = parse_name(substr($method, 10)); //todo 这个需要处理
            $where[$name] = $args[0];
            return $this->where($where)->getField($args[1]); //todo 这个需要处理
    ...
    public function addAll($dataList, $options = array(), $replace = false)
    {
        ...
        // 写入数据到数据库
        $result = $this->db->insertAll($dataList, $options, $replace); //todo 这个需要处理
        if (false !== $result) {
            $insertId = $this->getLastInsID();
            if ($insertId) {
                return $insertId;
    ...
    public function selectAdd($fields = '', $table = '', $options = array())
    {
        ...
        $options = $this->_parseOptions($options);
        // 写入数据到数据库
        if (false === $result = $this->db->selectInsert( //todo 这个需要处理
    ...
    {
        // 检查字段映射
        !defined('READ_DATA_MAP') && define('READ_DATA_MAP', true);
        if (!empty($this->_map) && READ_DATA_MAP) {
    ...
    public function create($data = '', $type = '')
    {
        ...
        if (isset($fields)) {
            ...
            // 判断令牌验证字段
            !defined('TOKEN_ON') && define('TOKEN_ON', true);
            !defined('TOKEN_NAME') && define('TOKEN_NAME', ''); //todo 没有懂这个什么意思
            if (TOKEN_ON) {
                $fields[] = TOKEN_NAME;
            }
            foreach ($data as $key => $val) {
    ...
    public function autoCheckToken($data)
    {
        ...
        !defined('TOKEN_ON') && define('TOKEN_ON', true);
        if (TOKEN_ON) {
            !defined('TOKEN_NAME') && define('TOKEN_NAME', '');
            $name = TOKEN_NAME ? TOKEN_NAME : '__hash__';
            ...
            // 开启TOKEN重置
            !defined('TOKEN_RESET') && define('TOKEN_RESET', true);
            if (TOKEN_RESET) {
    ...
    public function getTableName()
    {
        if (empty($this->trueTableName)) {
            ...
            if (!empty($this->tableName)) {
                ...
                $tableName .= parse_name($this->name); //todo 这个需要处理
            }
