...
class RedisDBBase extends BaseDBDAO
{
    ...
    protected function init($pk)
    {
        if ($this->needUseCache()) { // 需要使用redis 做为db缓存
            $this->redis = SMVCRedisHelper::getInstance();
        }
    }
    ...
    public function add($data = '', $options = [], $replace = false)
    {
        ...
        if ($this->needUseCache()) { // 需要使用redis 缓存
            if (is_int($return) && !isset($data[$this->_pk]) && $this->_pk_auto) {
                $data[$this->_pk] = $return;
            }
            if (isset($data[$this->_pk])) {
                ...
                if ($this->isMulit()) {
                    $sk = isset($data[$this->_sk]) ? $data[$this->_sk] : $return;
    ...
    public function getValidParentsByPath($path)
    {
        return $this->getValidParents('', $path);
    }

    public function getValidParentsById($id)
    {
        return $this->getValidParents($id, '');
    }
     ...
     * @return string
     */
    public function getValidParents($id, $path = '')
    {
        if (empty($path) && $id) {
            $currentInfo = $this->getOne(['id' => $id]);
            $path        = isset($currentInfo['path']) ? $currentInfo['path'] : '';
        }

        $where         = [];
        $where['path'] = ['NOTLIKE', sprintf('%%%s%%', $path)];
