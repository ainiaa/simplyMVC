...
class Config implements ArrayAccess, Iterator, Countable
{
     ...
     * @param array $info
     *
     * @internal param $configPath An optional absolute path to the configuration folder* An optional absolute path to the configuration folder
     *
     * @internal param $defaults An optional list of defaults to fall back on, set at instantiation* An optional list of defaults to fall back on, set at instantiation
     */
    public function __construct($info = array( /*'configPath' => '', 'defaults' => ''*/))
    {
        //Set the defaults
        $this->configSettings = isset($info['default']) ? $info['default'] : '';
        $configPath           = isset($info['configPath']) ? $info['configPath'] : '';
     ...
     *
     * @throws Exception
     * @internal param string $configPath An absolute path to the configuration folder* An absolute path to the configuration folder
     *
     * @return int
     * The number of configuration settings loaded
     */
    public function loadConfig($info = array( /*'configPath' => ''*/))
    {
     ...
     *
     * @throws Exception
     * @internal param string $filename The full path to the config file* The full path to the config file
     *
     * @return array
    ...
    public function parseConfigFile($filepath)
    {
        ...
        $parserClassname = ucfirst(strtolower(pathinfo($filepath, PATHINFO_EXTENSION)));
    ...
    private function mergeConfigArrays($arr1, $arr2)
    {
        foreach ($arr2 as $key => $value) {
            if (array_key_exists($key, $arr1) && is_array($value)) {
                $arr1[$key] = $this->mergeConfigArrays(
                        $arr1[$key],
                        $arr2[$key]
                );
            } else {
